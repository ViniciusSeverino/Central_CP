...
// events_shell.js).
import { app, escapeHtml, ROLE_LABEL } from './state.js';
...
export function renderConfiguracoes() {
  const active = app.state.configTab && CONFIG_TABS[app.state.configTab] ? app.state.configTab : 'cadastros';
  const topbar = `
    <div class="topbar">
    ...
    <div class="tabset" style="max-width:fit-content; padding:3px; margin-bottom:18px;">
      ${Object.entries(CONFIG_TABS).map(([key, label]) => `<button data-config-tab="${key}" class="${active === key ? 'active' : ''}" style="padding:8px 14px; flex:none;">${label}</button>`).join('')}
    </div>`;
